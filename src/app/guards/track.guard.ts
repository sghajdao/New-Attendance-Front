...
export const trackGuard: CanActivateFn = (route, state) => {
  ...
  return service.isAuthenticated().pipe(map((res) => {
    if (res) {
      ...
      if (res.authorities[0].attributes.email === 'D.Chayabaynou@aui.ma' || res.authorities[0].attributes.email === 'S.Ghajdaoui@aui.ma' || res.authorities[0].attributes.email === 'A.Bettahi@aui.ma')
        return true
      else {
