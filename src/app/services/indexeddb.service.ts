...
export class IndexeddbService {
  ...
  private async initDB() {
    return openDB('StudentsInfoDB', 1, {
      upgrade(db) {
        ...
        if (!db.objectStoreNames.contains('SU')) {
          const store = db.createObjectStore('SU', {
            keyPath: 'id',
            autoIncrement: true,
          });
          store.createIndex('list', 'id');
  ...
  async getByStudentId(studentId: string, termCode: string): Promise<StudentAttendanceDetails[]> {
    const db = await this.dbPromise;
